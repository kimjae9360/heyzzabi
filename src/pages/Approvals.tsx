        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="border-b border-gray-100 bg-amber-50 px-6 py-4 flex items-center justify-between">
              ...
              <Link to="/pipeline" className="text-xs font-bold text-amber-700 hover:underline">파이프라인 보기 →</Link>
            </div>
            {pendingTasks.length === 0 ? (
              <div className="py-14 text-center">
                ...
                <Link to="/meetings" className="mt-2 inline-block text-xs text-blue-500 font-bold hover:underline">회의분석에서 기획서 생성하기 →</Link>
              </div>
            ) : (
                ...
                {delayedTasks.map(task => {
                  ...
                  return (
                    <div key={task.id} className="px-6 py-4 flex items-center gap-4">
                      ...
                      <Link to="/pipeline" className="shrink-0 text-xs font-bold text-white bg-red-600 hover:bg-red-700 px-3 py-1.5 rounded-lg transition-colors">
                        재조정 실행 →
                      </Link>
                    </div>
                  );
