...
import { Briefcase, Search, CheckCircle2, AlertCircle, Clock, AlertTriangle, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function Tasks() {
  const { tasks, employees, updateTaskProgress } = useAppStore();
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [tasks, query, filterStatus, sortKey, sortAsc]);

  const statusCounts = {
    all: tasks.length,
    'pending-distribution': tasks.filter(t => t.status === 'pending-distribution').length,
